...
# usage
def usage():
    print("Usage: " + sys.argv[0] + " -i <input json file> -p <cracked pot file>", file=sys.stderr) 
    sys.exit(1)
...
def pxor(pwd_unencoded,share):
    pwd = pwd_unencoded.encode('utf-8')
    words=share.split("-")
    hexshare=words[1]
    slen=len(hexshare)
    print("RORY")
...
def decrypt(enc, password):
    ...
    password = bytes.fromhex(password.zfill(32)).decode('utf-8')
